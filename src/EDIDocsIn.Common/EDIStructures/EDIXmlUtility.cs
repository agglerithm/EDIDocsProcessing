...
namespace EDIDocsProcessing.Core.DocsOut.EDIStructures
{
    ...
    public class EDIXmlUtility
    {
        ...
        */
        public static IEnumerable<XElement> get_elements_by_label(XElement elm,
                                                                      string el_name, string spec_name)
        {
            var els = from e in elm.Elements(el_name)
                      where e.Attribute(EdiStructureNameConstants.Label).GetSafeValue() == spec_name
                      select e; 
            return els;
        }

        public static IEnumerable<XElement> get_descendants_by_label(XElement elm,
                                                                         string el_name, string spec_name)
        {
        ...
        public static XElement get_element_by_label(XElement elm,
                                                        string el_name, string spec_name)
        {
            var els = from e in elm.Elements(el_name)
                      where e.Attribute(EdiStructureNameConstants.Label).GetSafeValue() == spec_name
        ...
        public static XElement get_descendant_by_label(XElement elm,
                                                           string el_name, string spec_name)
        {
            var els = from e in elm.Descendants(el_name)
                      where e.Attribute(EdiStructureNameConstants.Label).GetSafeValue() == spec_name
